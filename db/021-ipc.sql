...
-- -- create function IPC.rpc(                method text, parameters anyelement ) returns jsonb volatile language plpgsql as $$ begin return IPC._rpc( 'all',   method, to_jsonb( parameters       ) ); end; $$;
-- -- create function IPC.rpc(                method text, parameters text       ) returns jsonb volatile language plpgsql as $$ begin return IPC._rpc( 'all',   method, to_jsonb( parameters       ) ); end; $$;
-- -- create function IPC.rpc(                method text, parameters unknown    ) returns jsonb volatile language plpgsql as $$ begin return IPC._rpc( 'all',   method, to_jsonb( parameters::text ) ); end; $$;
