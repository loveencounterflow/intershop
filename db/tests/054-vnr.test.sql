...
-- ---------------------------------------------------------------------------------------------------------
create function VNR.push_zero( ¶vnr integer[] )
  returns integer[] immutable parallel safe language sql as $$
  select VNR.push_zero( ¶vnr::VNR.vnr )::integer[]; $$;

-- ---------------------------------------------------------------------------------------------------------
create table VNR.vnr_float8 (
  nr        integer generated by default as identity primary key,
  vnr       VNR.vnr not null,
  _vnr0     VNR.vnr generated always as ( VNR.push_zero( vnr ) ) stored,
...
create table VNR.vnr_integer (
  ...
  _vnr0      integer[] generated always as ( VNR.push_zero( vnr ) ) stored,
...
-- create unique index on VNR.vnr_integer using btree ( VNR.push_zero( vnr ) );
create unique index on VNR.vnr_integer ( VNR.push_zero( vnr ) );
...
select * from VNR.datoms order by _vnr0;
-- select * from VNR.vnr_float8 order by VNR.push_zero( vnr );
-- explain analyze select * from VNR.vnr_float8 order by VNR.push_zero( vnr );
