...
does not exist`);
  ...
  this.find_addons = function() {
    ...
    R = this._find_addons('guest', process.env.intershop_guest_path);
    echo('^76765^', CND.inspect(R));
    R = this._find_addons('host', process.env.intershop_host_path);
    echo('^76765^', CND.inspect(R));
    return R;
  };

  //-----------------------------------------------------------------------------------------------------------
  this._find_addons = function(location, XXX_path) {
    var R, addon, addons, aoid, error, file_id, ipj, package_json, path, ref, ref1, relpath, target, type, version;
    debug('^76765^', {location, XXX_path});
